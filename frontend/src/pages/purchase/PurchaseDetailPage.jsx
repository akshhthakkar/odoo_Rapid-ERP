...
// ─── AUDIT TIMELINE ───────────────────────────────────────────────────────────
const AUDIT_ICONS = {
  PURCHASE_ORDER_CREATED:  { icon: FileText, color: '#FF540E' },
  PURCHASE_ORDER_SENT:     { icon: CheckCircle2, color: '#60A5FA' },
  PURCHASE_RECEIPT_CREATED:{ icon: ArrowDownToLine, color: '#FB923C' },
  PURCHASE_ORDER_RECEIVED: { icon: Award, color: '#4ADE80' },
  PURCHASE_ORDER_CANCELLED: { icon: XCircle, color: '#F87171' },
};
  ...
  return (
    <div>
      {timeline.map((entry, idx) => {
        const cfg = AUDIT_ICONS[entry.action] || { icon: ClipboardList, color: '#94A3B8' };
        const IconComponent = cfg.icon;
        return (
          <div key={entry.id} style={{ display: 'flex', gap: '12px', marginBottom: idx < timeline.length - 1 ? '12px' : 0 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: `${cfg.color}18`, border: `1px solid ${cfg.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <IconComponent size={14} style={{ color: cfg.color }} />
              </div>
      ...
      <div style={modalStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <ArrowDownToLine size={16} /> Receive Goods — {po.orderRef}
          </h3>
  ...
  return (
    <div className="animate-fade-in" style={{ fontFamily: 'var(--font-family)', maxWidth: '1040px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap', marginBottom: '4px' }}>
            <h2 style={{ fontSize: '22px', fontWeight: 800, color: 'var(--accent)', margin: 0 }}>{po.orderRef}</h2>
            <StatusBadge status={po.status} />
            {po.isAutoGenerated && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: 'rgba(255,84,14,0.08)', color: '#FF540E', border: '1px solid rgba(255,84,14,0.2)', borderRadius: '20px', padding: '2px 10px', fontSize: '11px', fontWeight: 700 }}>
      ...
      {po.salesOrderId && po.salesOrderRef && (
        <div
          style={{ background: 'rgba(255,84,14,0.08)', border: '1px solid rgba(255,84,14,0.2)', borderRadius: '10px', padding: '12px 18px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}
          onClick={() => navigate(`/sales/${po.salesOrderId}`)}
        >
          <Zap size={16} style={{ color: '#FF540E' }} />
          <span style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
            AUTO GENERATED — Created from Sales Order{' '}
            <span style={{ fontWeight: 700, color: 'var(--accent)', textDecoration: 'underline' }}>
              {po.salesOrderRef}
            </span>
          </span>
          <span style={{ marginLeft: 'auto', fontSize: '11px', color: 'var(--accent)' }}>View SO <ArrowRight size={11} style={{ display: 'inline', verticalAlign: 'middle' }} /></span>
        </div>
      )}
          ...
          <div className="glass-card" style={{ padding: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
              <div>
                ...
                {po.vendorEmail && <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px', display: 'flex', alignItems: 'center', gap: '6px' }}><Mail size={12} /> {po.vendorEmail}</p>}
                {po.vendorPhone && <p style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '1px', display: 'flex', alignItems: 'center', gap: '6px' }}><Phone size={12} /> {po.vendorPhone}</p>}
              </div>
              <div>
              ...
              <div>
                ...
                <p style={{ ...metaValue, fontWeight: 700, color: 'var(--accent)', fontSize: '18px' }}>₹{(po.totalCost || 0).toFixed(2)}</p>
                ...
                {po.receipts.map((r, idx) => (
                  <div key={r.id} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px 16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ background: 'rgba(255,84,14,0.08)', color: '#FF540E', border: '1px solid rgba(255,84,14,0.2)', borderRadius: '20px', padding: '2px 10px', fontSize: '11px', fontWeight: 700 }}>
                          Receipt #{idx + 1}
                        </span>
          ...
          <div className="glass-card" style={{ padding: '20px' }}>
            ...
            {['SENT', 'PARTIALLY_RECEIVED'].includes(po.status) && canReceive && hasRemainingLines && (
              <Button
                ...
                style={{ width: '100%', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
              >
                <ArrowDownToLine size={14} /> Receive Goods
              </Button>
            )}
            ...
            {po.status === 'RECEIVED' && (
              <div style={{ textAlign: 'center', padding: '12px', background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '8px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                <Award size={32} style={{ color: '#4ADE80', marginBottom: '6px' }} />
            ...
            {['DRAFT', 'SENT', 'PARTIALLY_RECEIVED', 'RECEIVED'].map((s, i) => {
              ...
              return (
                <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: i < 3 ? '10px' : 0 }}>
                  <div style={{ width: '22px', height: '22px', borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 700, background: isDone ? (isCurrent ? 'rgba(255,84,14,0.15)' : 'rgba(34,197,94,0.15)') : 'rgba(255,255,255,0.04)', border: `2px solid ${isDone ? (isCurrent ? '#FF540E' : '#4ADE80') : 'var(--border)'}`, color: isDone ? (isCurrent ? '#FF540E' : '#4ADE80') : 'var(--text-muted)' }}>
