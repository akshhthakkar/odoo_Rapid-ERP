...
export const listBoms = async (req, res, next) => {
  try {
    const boms = await bomService.getBoms(req.query);
...
export const getBom = async (req, res, next) => {
  try {
    ...
    if (isNaN(id)) {
      throw { status: 400, message: 'Invalid Bill of Materials ID' };
    }
...
export const getBomByProduct = async (req, res, next) => {
  try {
    ...
    const bom = await bomService.getBomByProductId(productId);
    res.status(200).json(bom);
  } catch (err) {
    next(err);
  }
...
export const createBom = async (req, res, next) => {
  try {
    const bom = await bomService.createBom(req.body, req.user.id);
...
export const updateBom = async (req, res, next) => {
  try {
    ...
    const bom = await bomService.updateBom(id, req.body, req.user.id);
...
export const deleteBom = async (req, res, next) => {
  try {
    ...
    const result = await bomService.softDeleteBom(id, req.user.id);
